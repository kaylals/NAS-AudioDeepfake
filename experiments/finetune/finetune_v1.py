...
if __name__ == '__main__':
    ...
    if args.pretrained_model:
        if args.finetune_lr:
            ...
            print(f"🎯 Using fine-tuning learning rate: {args.lr}")
        else:
            args.lr *= 0.1
            print(f"🎯 Using default 1/10 learning rate for fine-tuning: {args.lr}")
    ...
    for epoch in range(args.num_epochs):
        ...
        # validation - 现在返回准确率和EER
        if epoch % args.valid_freq == 0:
            dev_acc, dev_eer = validate(dev_loader, model, criterion, epoch, writer_dict, validate_type='dev')
            
            logging.info('dev_acc %f', dev_acc)
            logging.info('dev_eer %f', dev_eer)
            
            # 可以根据EER或准确率保存最佳模型
            if dev_acc > best_acc:  # 或者使用 dev_eer < best_eer
                best_acc = dev_acc
